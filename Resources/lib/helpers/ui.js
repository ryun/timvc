t$.ui.truncate = function(txt, maxlen, suffix){
	maxlen = maxlen || 35;
	suffix = suffix || '...';
	if (txt && txt.length > maxlen) return txt.substr(0,maxlen) + suffix;
	return txt;
};
...
t$.ui.addToView = function(view, element) {
	...
	{
		for (var i in element) { view.add(element[i]); }
	} else {
		view.add(element);
	}
};
...
t$.ui.busy.prototype = {
	...
	show: function showIndicator(msg) {
		...
		{
			// window container
			this.winInd = Titanium.UI.createWindow({height:150,	width:150});
			...
			this.myInd = Titanium.UI.createActivityIndicator({style:Titanium.UI.iPhone.ActivityIndicatorStyle.BIG,height:30,width:30});
			this.message = Titanium.UI.createLabel({
				text: msg,
				color:'#fff',width:'auto',height:'auto',font:{fontSize:20,fontWeight:'bold'},bottom:20
			});
			this.winInd.add(this.message);
			this.winInd.open();
		} else {
			this.myInd = Titanium.UI.createActivityIndicator({height:30,width:30});
			this.myInd.message = msg;
		}
		this.myInd.show();
	},
	update: function(msg){
		if (!t$.isAndroid) this.message.text = msg;
		else this.myInd.message = msg;
	},
	hide: function() {
		this.myInd.hide();
		if (!t$.isAndroid) {
			this.winInd.close({opacity:0,duration:500});
		}
	}
};